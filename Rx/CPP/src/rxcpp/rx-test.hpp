    ...
    : public std::enable_shared_from_this<test_subject_base<T>>
{
    ...
    virtual void on_subscribe(observer<T, dynamic_observer<T>>) const =0;
...
class testable_observer 
    : public observer<T, dynamic_observer<T>>
{
    typedef observer<T, dynamic_observer<T>> observer_base;
